...
cols_to_drop = ['Margin_Type_without', 'User_Name', 'Side', 'Tumor_depth', 'Nodes_exam',
                ...
                "Diagnosis_date", "Surgery_date1", "Surgery_date2", "Surgery_date3", "Surgery_name1", "Surgery_name2", "Surgery_name3"]
...
def preprocessing_train(df: pd.DataFrame, labels: pd.DataFrame, multi_label=True):
    ...
    learned_consts["KI67_protein"] = df['KI67_protein'].mean()
    df['KI67_protein'].fillna(learned_consts["KI67_protein"], inplace=True)
...
#     return df, ids_vector


def preprocessing_test(df: pd.DataFrame):
    ...
    df['pr'] = df['pr'].apply(clean_er_pr)
    df.drop(cols_to_drop, axis=1, inplace=True)
    df.drop(["idhushed_internalpatientid"], axis=1, inplace=True)
